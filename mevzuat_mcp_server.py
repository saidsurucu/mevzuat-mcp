...
async def search_mevzuat(
    ...
    phrase: Optional[str] = Field(None, description="Turkish search phrase. Working operators: `+` (required term), `-` (exclude term), `*` (wildcard), `\" \"` (exact phrase), `( )` (grouping), `~` (fuzzy search), `/pattern/` (basic regex with `.` any char, `+` one or more, `?` optional, `[abc]` character class, `{n,m}` repetition, `|` alternation); Not working: AND, OR, NOT boolean operators, `^$` anchors, `\\b` word boundaries, `()` regex groups, `\\d\\w\\s` special chars, `[TO]` range queries."),
